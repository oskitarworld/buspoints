/*
Apply remote href iconPath updates from dry-run CSV.
- Reads CSV (default tool/dryrun_icon_map_local.csv)
- Selects rows where proposedType==remote_href and matchedDocId is set
...
Usage:
  node tool/apply_remote_href_from_csv.js --serviceAccount=tool/sa.json --csv=tool/dryrun_icon_map_local.csv --dryRun=false
...
(async ()=>{
  try {
    ...
    const toApply = rows.filter(r => r.proposedType==='remote_href' && r.matchedDocId);
    console.log('Rows to apply (remote_href & matchedDocId):', toApply.length);
    ...
    const backupPath = path.join(backupDir, `pdis_full_icon_backup_remote_${ts}.json`);
    ...
    for (let i=0;i<ids.length;i++) {
      ...
      if (doc.exists) backupDocs[id] = doc.data(); else backupDocs[id] = null;
    }
    fs.writeFileSync(backupPath, JSON.stringify(backupDocs,null,2),'utf8');
    console.log('Wrote backup to', backupPath);

    if (dryRun) {
      console.log('Dry run enabled — no changes applied. To apply, rerun with --dryRun=false');
